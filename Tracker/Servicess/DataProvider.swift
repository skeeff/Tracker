...
// MARK: - Делегаты TrackerCategoryStore, TrackerStore, TrackerRecord
extension DataProvider: TrackerCategoryStoreDelegate {
    func didUpdateCategories(newCategories: [TrackerCategory]) {
        self.categories = newCategories // Обновляем локальное свойство
        delegate?.didUpdate() // Сообщаем ViewModel об обновлении
    }
}

extension DataProvider: TrackerStoreDelegate {
    func didUpdateTracker() {
        // TrackerStore не предоставляет категории, поэтому мы просто
        // получаем их заново и сообщаем об обновлении
        self.categories = categoryStore.categories
...
//        self.delegate?.didUpdateCategories(newCategories: newCategories)
//    }
//}
////extension DataProvider: TrackerCategoryStoreDelegate {
